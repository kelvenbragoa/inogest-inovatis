	...
	<body>
		<div class="invoice-box">
			<table cellpadding="2" cellspacing="0">
					...
					<td colspan="4">
						<table>
							<tr>
								...
								<td>
									{{$invoice->quotation->customer->name}}<br />
									{{$invoice->quotation->representative}}<br />
									{{$invoice->quotation->customer->mobile}}<br />
									{{$invoice->quotation->customer->email}}<br />
				...
				<tr class="details">
					<td colspan="4">{{$invoice->quotation->service}}</td>
				</tr>
				...
				<?php $sum_total = 0 ?>
				@foreach ($invoice->quotation->quotation_detail as $item)
				<tr class="item">
					<td>{{$item->name}}</td>
					<td>{{$item->quatity}}</td>
					<td>{{$item->price}}</td>
				...
				<tr class="total">
					...
					<td colspan="4">IVA(17%): {{$sum_total*0.17}}MT</td>
				</tr>

				<tr class="total">
					

					<td colspan="4">Total(Com IVA incluído): {{$sum_total+($sum_total*0.17)}}MT</td>
				</tr>
				<tr class="total">
					

					<td colspan="4">Obs.:{{$invoice->obs}}</td>
				</tr>
